 ...
 *  The keys are case-insensitive NULL terminated strings, and
 *  the data is arbitrary lumps of data.  Copies of both the
...
hashmap_t hashmap_create (unsigned int nbuckets)
{
    ...
    ptr->size = nbuckets;
    ptr->buckets = (struct hashbucket_s *) \
		safecalloc (nbuckets, sizeof (struct hashbucket_s));
    if (!ptr->buckets)
    {
        safefree (ptr);
        return NULL;
...
hashmap_iter hashmap_find (hashmap_t map, const char *key)
{
    ...
    for (i = 0; i != map->size; i++)
    {
        ...
        while (ptr)
        {
            if (proxy_strcasecmp (ptr->key, key) == 0)
            {
